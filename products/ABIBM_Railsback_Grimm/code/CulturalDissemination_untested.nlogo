...
The grid of sites is 10 x 10 patches in extent, with the space not wrapped. The distance between sites is not stated, and in fact it is not clear that the grid represents geographic space. Time is represented only by the number of 'ticks' (times that the schedule is executed); each tick does not represent a specific amount of time. Model runs continue until the system is stable, defined as executing 1000 consecutive ticks with no change in site state variables. Reaching this stable end can take more than 80,000 ticks.
...
The single submodel is the trait a site uses to interact culturally with a neighbor site. It has these steps (â€œthe siteâ€ refers to the patch executing this trait):

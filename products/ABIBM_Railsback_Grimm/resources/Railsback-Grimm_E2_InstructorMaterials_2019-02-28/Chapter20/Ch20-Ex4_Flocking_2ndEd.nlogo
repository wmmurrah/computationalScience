globals [ ;; New calibration variables for exercise 4 of chapter 20, Railsback & Grimm
  nnd
  mnd
  polarization
]
...
to setup
  clear-all
  let min-start-x 0 - (max-pxcor / 5)
  let min-start-y 0 - (max-pycor / 5)

  create-turtles population
    [ set color yellow - 2 + random 7  ;; random shades look nice
      set size 1.5  ;; easier to see
      setxy (min-start-x + random-float (max-pxcor / 2.5)) (min-start-y + random-float (max-pycor / 2.5))
...
to-report average-heading  ;; Now an Observer procedure to report
  ;; mean heading of all turtles
...
to update-output ; A new global procedure to calculate calibration variables

  set nnd mean [min [distance myself] of other turtles] of turtles

  set mnd max [max [distance myself] of other turtles] of turtles

  let mean-heading average-heading

  set polarization mean [abs subtract-headings heading mean-heading] of turtles
...
**This version is modified as an example solution to Exercise 4, Chapter 20, of Railsback & Grimm, 2nd Edition. This file is provided as instructor materials for Chapter 20 of _Agent-based and Individual-based Modeling, 2nd edition_, by Railsback and Grimm (2019). Please do not copy or distribute this file. It is available upon request from www.railsback-grimm-abm-book.com.**
...
@#$#@#$#@
<experiments>
  <experiment name="CalibrationExercise" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <final>set nnd mean [min [distance myself] of other turtles] of turtles
set mnd max [max [distance myself] of other turtles] of turtles
let mean-heading average-heading
set polarization mean 
[abs subtract-headings heading mean-heading] of turtles</final>
    ...
    <steppedValueSet variable="max-cohere-turn" first="0" step="5" last="20"/>
    <steppedValueSet variable="max-separate-turn" first="0" step="5" last="20"/>
    <steppedValueSet variable="vision" first="1" step="3" last="10"/>
    <steppedValueSet variable="minimum-separation" first="0" step="1.25" last="5"/>
    <steppedValueSet variable="max-align-turn" first="0" step="5" last="20"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@

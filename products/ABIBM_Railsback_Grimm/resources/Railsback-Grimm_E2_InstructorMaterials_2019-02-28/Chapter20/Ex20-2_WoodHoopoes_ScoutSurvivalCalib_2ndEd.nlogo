...
to-report I-should-scout  ; a turtle reporter, returns a boolean
  ...
  ifelse count (other turtles-here) with
   [
     (is-female? = [is-female?] of myself) and
     (not is-alpha?) and
     (age-in-months > [age-in-months] of myself)
   ] > 1
  [report random-bernoulli scout-prob] ; There are more than 1 older siblings of same sex
  [report false] ; There are no more than 1 older siblings of same sex
...
@#$#@#$#@
# Ex20-2_WoodHoopoes_ScoutSurvivalCalib_2ndEd

This file is provided as instructor materials for Chapter 20 of _Agent-based and Individual-based Modeling, 2nd edition_, by Railsback and Grimm (2019). Please do not copy or distribute this file. It is available upon request from www.railsback-grimm-abm-book.com.

This file is copyrighted 2019 by Steven F. Railsback and Volker Grimm.

This is the simplified Woodhoopoe model from Section 19.4.3. The key adaptive trait of scouting for vacant territories is the simple version described in Section 20.5 for an exercise in model calibration. 
...
@#$#@#$#@
<experiments>
  <experiment name="Calibration-ScoutSurv" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>year</metric>
    <metric>month</metric>
    ...
    <steppedValueSet variable="scouting-survival" first="0.5" step="0.05" last="1"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@

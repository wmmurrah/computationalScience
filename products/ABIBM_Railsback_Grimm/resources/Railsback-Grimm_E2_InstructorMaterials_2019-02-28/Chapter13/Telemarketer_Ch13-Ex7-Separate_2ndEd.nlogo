...
;   set money-size-ratio 0.001  --Moved to slider
   ...
   [
     ...
     set call-tolerance random-float 0.5  ; Tolerance is between 0.0 and 0.5
   ]
   ...
   crt initial-num-marketers
     ...
     [
       ...
       set do-not-call-list []  ; initialize this list
     ]
...
to do-sales
   ...
   ; Patches on do-not-call list do not count as potential customers
   let potential-customers patches in-radius (10 * size ^ 0.5)
   ; write (word "total: " count potential-customers)
   set potential-customers potential-customers with [not member? self [do-not-call-list] of myself ]
   ; write (word " Not on list: " count potential-customers)
   ; print (word " Num on list: " length do-not-call-list)
   ...
   let rejecters customers-called with [pcolor = brown]
   ...
   ; Now let annoyed customers decide whether to be on do-not-call list
   ask rejecters [reject-call-from myself]
...
to reject-call-from [a-marketer]  ; A patch procedure to decide whether to be on do-not-call list

  if random-float 1.0 < call-tolerance
  [
    ask a-marketer [ set do-not-call-list fput myself do-not-call-list]
  ]
...
@#$#@#$#@
# Telemarketer_Ch13-Ex7-Separate_2ndEd.nlogo
...
This version is the first, basic version of the model as described in Section 13.3, but modified for Exercise 7: requiring each telemarketer to keep a do-not-call list. The modifications are described throughout this ODD description.
...
Potential customers have two state variables in addition to their location. One is a tolerance for being called (described below at submodels). Second is their availability variable, indicating whether the customer is in one of two states: not yet called but available this time step, or already called this time step. (In the NetLogo program you can use the patch's color for this variable: the patch is one color if it has not yet been called and a second color if it has been called and a third color if it is on the do-not-call list.)
...
In this version, each telemarketer also has a list of customers that have asked it not to ever call them again.
...
The potential customers have their availability variable initialized to indicate that they have not yet been called this week. Their tolerance variable is drawn from a uniform floating-point distribution with values between 0.0 and 0.5.
...
In this version, some potential customers are on the telemarketer's own do-not-call list and therefore not available to receive sales calls or buy anything. These customers are not included in the potential customers that telemarketers might call, and do not count toward the maximum number of calls per week.
...
**Customer reject-call.** When a customer receives a call on a time step in which it has already been called and bought something, it does not buy anything. The customer also decides randomly whether to have the telemarketer calling put the customer on the telemarketer's do-not-call list, so that telemarketer can never call the customer again. This decision is stochastic, with the probability of asking not to be called again equal to the customer's tolerance variable.

...
patches-own
[
  weekly-call-tolerance
  times-called-this-week
]
...
;   set money-size-ratio 0.001  --Moved to slider
   ...
   ask patches
   [
     set pcolor green  ; to indicate "not called yet but available"
     set weekly-call-tolerance 1 + random 4  ; Tolerance is between 1 and 4
     set times-called-this-week 0
   ]
...
to go
  ...
  ask patches  ; Reset the patch variables
  [
...
;  file-close
  ...
  ask patches [update-availability]  ; Patches decide whether to be on do-not-call list
...
to do-sales
   ...
   ; Patches on do-not-call list are red; they do not count as potential customers
   let potential-customers patches in-radius (10 * size ^ 0.5) with [pcolor != red]
   ...
   ; Now update the times-called of all patches called
   ask customers-called [set times-called-this-week (times-called-this-week + 1)]
...
to update-availability  ; A patch procedure to decide whether to be on do-not-call list

  if pcolor = red [stop]

  if times-called-this-week > weekly-call-tolerance [ set pcolor red ]
...
@#$#@#$#@
# Telemarketer_Ch13-Ex7-National_2ndEd.nlogo

This file is provided as instructor materials for Chapter 13 of _Agent-based and Individual-based Modeling, 2nd edition_, by Railsback and Grimm (2019). Please do not copy or distribute this file. It is available upon request from www.railsback-grimm-abm-book.com.

This file is copyrighted 2019 by Steven F. Railsback and Volker Grimm.

## MODEL DESCRIPTION (ODD)

This version is the first, basic version of the model as described in Section 13.3, but modified for Exercise 7: implementing a "national" do-not-call list. The modifications are described throughout this ODD description.
...
Potential customers have three state variables in addition to their location. One is an integer count of the number of times they have been called by a telemarketer during the current time step. The second variable is their tolerance for being called (described below at submodels). Third is their availability variable, indicating whether the customer is in one of three states: not yet called but available this time step, already called this time step, or on a do-not-call list and hence never available. (In the NetLogo program you can use the patch's color for this variable: the patch is one color if it has not yet been called and a second color if it has been called and a third color if it is on the do-not-call list.)
...
First, the patches re-set their state variable for whether they have been called by a telemarketer this week to "no" (unless they are on the do-not-call list). They also re-set their variable for how many times they have been called this step to zero.

Second, the telemarketers all do their sales calls (described below at **Submodels**). Each marketer calls a number of potential customers; this number increases with telemarketer size. The potential customers are within a radius of the telemarketer that increases with the marketer's size. (In this early stage of Wasellya's development, the cost of phone calls increases steeply with their distance.) Each potential customer buys something if it has not been called previously in the time step, then sets its variable for whether it has been called to "yes". If that variable was already "yes", the potential customer hangs up on the telemarketer and buys nothing. (Wasellyans are not very sophisticated yet, but they have short tempers.) Each time a customer is called, it increments its variable for how many times it has been called this step.
...
Third, the telemarketers do their weekly accounting (described below at **Submodels**). Income is determined from the number of successful calls. Costs of business increase linearly with size. The bank balance state variable is updated by adding sales income and subtracting costs. If the bank balance is negative, the telemarketer goes out of business and is removed from the model. Finally, the telemarketer increases its size if its bank balance is high enough.

Fourth, the customers decide whether to put themselves on the do-not-call list. If their variable for number of times called exceeds their tolerance threshold, they put themselves on the do-not-call list by changing their availability variable.

The fifth action is observer updates: outputs such as the number of telemarketers still in business, a histogram of their size, and the total number of sales, are reported.
...
In this version, customers adapt to increasing frequency of being called by placing themselves on a do-not-call list. 
...
The model is initialized with 200 telemarketers, each with size 1.0 and bank balance of 0.0, and a location chosen randomly from all the points in the space, with equal probability for all points. (For display purposes, these are given random colors and drawn as circles.) 

The potential customers have their availability variable initialized to indicate that they have not yet been called this week. Their tolerance variable is drawn from a uniform integer distribution with values from 1 to 4. The number of times called this week is set to zero.
...
In this version, some potential customers are on the do-not-call list and therefore not available to receive sales calls or buy anything. These customers are not included in the potential customers that telemarketers might call, and do not count toward the maximum number of calls per week.

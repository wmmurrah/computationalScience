...
to do-accounting
   ...
   ifelse bank-balance > growth-param
    ...
    [
      if bank-balance < 0  ; Now try merging to avoid going out of business
       [
         ifelse any? my-out-links
          [beg-from one-of out-link-neighbors] ; "beg-from" gets money from the parent
          [
            let turtles-to-merge-with other turtles with [size > [size] of myself and
                bank-balance > (-1 * [bank-balance] of myself)]
         ...
         if bank-balance < 0  [die]    ; Could not merge or beg, so go out of business
       ]
    ]
...
@#$#@#$#@
# Telemarketer-Mergers-Links_Ch13-Ex5_2ndEd.nlogo
...
This code implements the version of Section 13.5, in which failing telemarketers merge with others. This new submodel is not documented here.

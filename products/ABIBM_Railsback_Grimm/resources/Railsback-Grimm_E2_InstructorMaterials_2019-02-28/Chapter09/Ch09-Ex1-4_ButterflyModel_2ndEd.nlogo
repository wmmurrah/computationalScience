; globals [ q ]      ;  Moved to slider on the Interface
                     ; q is the probability that a butterfly moves
                     ; intentionally to the highest surrounding patch
patches-own
[
  elevation
  used? ; A boolean variable for whether any butterflies occupied the patch
]

turtles-own [ start-patch ]  ; A variable containing the patch where butterfly started

to setup

  ca

  ; Patch elevations are now read in from the file "ElevationData.txt"
  ...
  ; Calculate the min and max elevation for shading patches
  ...
  ;  Create 50 butterflies now
  crt 50
   [
     set size 2
     ; Set initial location of butterflies - now, randomly within a 10x10 area
     setxy (100 + random 10) (130 + random 10)
     pen-down

     ; Have butterflies remember where they started
   ...
   ; Update the plots so we see the starting conditions
   update-outputs
...
to go  ;  This is the master schedule
  ...
  ; Update outputs here so they get updated on tick 1000
  update-outputs
...
to move  ; The butterfly move procedure, in turtle context
  ...
  ; NO LONGER stop if already on a hilltop
  ...
  ; For Exercise 1 of Ch. 9, identify the patch to move to so turtle can face it
...
to-report corridor-width    ; A reporter to calculate corridor width, in observer context

  let num-patches-visited count patches with [used?] ; equivalent to: "with [used? = true]"
...
to update-outputs
    ...
    ; For Exercise 3 of Chapter 9, plot mean elevation of butterflies
    set-current-plot "Mean butterfly elevation"
    plot mean [elevation] of turtles

    ; For Exercise 4 of Chapter 9, histogram butterfly elevations
    ; Set the X axis range here, from 0 to the multiple of 10 above the
    ; highest patch elevation
    set-current-plot "Butterfly elevation histogram"
    set-plot-x-range 0 (precision (max [elevation] of patches + 10) -2)
...
This file is provided as instructor materials for Chapter 9 of _Agent-based and Individual-based Modeling, 2nd edition_, by Railsback and Grimm (2019). Please do not copy or distribute this file. It is available upon request from www.railsback-grimm-abm-book.com.

This file is copyrighted 2019 by Steven F. Railsback and Volker Grimm.

The file implements the Butterfly model with the changes described in exercises 1-4 of Chapter 9.

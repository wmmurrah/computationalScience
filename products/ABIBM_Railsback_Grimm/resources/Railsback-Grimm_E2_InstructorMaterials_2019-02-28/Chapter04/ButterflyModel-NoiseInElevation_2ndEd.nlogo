...
to setup
  ...
  [
    ...
    set elevation 200 + (100 * (sin (pxcor * 4.0) + sin (pycor * 4.0)))

    set elevation elevation + random-float 50.0
...
This file is provided as instructor materials for Chapter 4 of _Agent-based and Individual-based Modeling, 2nd edition_, by Railsback and Grimm (2019). Please do not copy or distribute this file. It is available upon request from www.railsback-grimm-abm-book.com.

This file is copyrighted 2019 by Steven F. Railsback and Volker Grimm.

The file implements the Butterfly model of Chapter 4. The following model description is from Section 3.4 of Railsback and Grimm (2019). 

This version implements the "noise" in landscape elevation referred to in exercise 4 of chapter 4.
...
## 5. Initialization
The topography of the landscape (the elevation of each patch) is initialized when the model starts. Two kinds of landscapes are used in different versions of the model: (1) a simple artificial topography, and (2) the topography of a real study site, imported from a file containing elevation values for each patch. This version of the model uses the simple artificial topography _but_ adds noise to the landscape by adding a different random real number between 0.0 and 50 to the elevation of each patch.

The butterflies are initialized by creating five hundred of them and dispersing them throughout the landscape: each butterfly’s initial location is set to a patch selected randomly from among all patches.

globals
 [
   ...
   ; Output variables
   num-failures  ; Number of times an investor failed
  ]
...
to setup
 ...
 ; Initialize global variables
  ...
  set num-failures 0 ; Number of times an investor failed

 ; Initialize patch profit and risk characteristics
  ask patches
    [
     ; Set the profitability
     set profit (random-exponential patch-mean-profit) - 2000
...
to reposition
  ...
  ; Identify the best one of the destinations. Now there are three alternative
  ; objective functions--uncomment the one you want. Be sure to also
  ; change the "set current-utility" statement in do-accounting.
  ; let best-patch max-one-of potential-destinations [utility-for myself]
  ; let best-patch max-one-of potential-destinations [profit-for myself]
   let best-patch max-one-of potential-destinations [safety-for myself]
...
to do-accounting
  ...
  ; Now see if the investment failed, either by chance or by having negative wealth
  if (random-float 1.0 < annual-risk) or (wealth < 0)
  [
    set wealth 0
  ...
  ; Uncomment the objective function now being used.
  ; set current-utility utility-for self
  ; set current-utility profit-for self
   set current-utility safety-for self
...
to-report utility-for [a-turtle]
 ...
 ; Set utility to zero if it is negative
 if utility < 0.0 [ set utility 0.0]
...
to-report profit-for [a-turtle]
 ; Edit this to change how turtles evaluate alternative patches
 ; This version of the objective function just maximizes income without
 ; regard for risk.
 ...
 ; Then factor in risk of failure over time horizon -- ignored here
 ; set utility utility * ((1 - annual-risk) ^ decision-time-horizon)
...
to-report safety-for [a-turtle]
 ; Edit this to change how turtles evaluate alternative patches
 ; This version of the objective function minimizes risk without
 ; regard for income or wealth.

 ; Calculate probability of not failing over time horizon
 let utility (1 - annual-risk) ^ decision-time-horizon

 ; Calculate expected wealth at end of time horizon
 let turtles-wealth [wealth] of a-turtle
 let expected-wealth turtles-wealth + (profit * decision-time-horizon)

 ; Set utility to zero if expected wealth is negative
 if expected-wealth < 0.0 [ set utility 0.0]
...
to output
    ...
    set-current-plot "Coefficient of Variation in Wealth"
    ; Coefficient of variation is standard deviation / mean.
    ; Avoid division by zero when mean wealth is zero (at time 0) by plotting a zero instead
    plot ifelse-value (mean-wealth > 0)
    [ (standard-deviation [wealth] of turtles) / mean-wealth ]
    [ 0 ]

    output-print (word "# failures: " num-failures)
...
443
471
Coefficient of Variation in Wealth
Year
COV
0.0
10.0
...
@#$#@#$#@
# BusinessInvestors_Sect11-5_2ndEd.nlogo

This file is provided as instructor materials for Chapter 11 of _Agent-based and Individual-based Modeling, 2nd edition_, by Railsback and Grimm (2019). Please do not copy or distribute this file. It is available upon request from www.railsback-grimm-abm-book.com.

This file is copyrighted 2019 by Steven F. Railsback and Volker Grimm.

This file implements the version of the Business Investor model described in Section 11.5, with negative profits and a revised objective function. This model description has not been updated with the changes described in Section 11.5.
...
@#$#@#$#@
<experiments>
  <experiment name="Standard" repetitions="1000" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean [wealth] of turtles</metric>
    <metric>num-failures</metric>
  </experiment>
  <experiment name="Profit" repetitions="1000" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean [wealth] of turtles</metric>
    <metric>num-failures</metric>
  </experiment>
  <experiment name="Safety" repetitions="1000" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean [wealth] of turtles</metric>
    <metric>num-failures</metric>

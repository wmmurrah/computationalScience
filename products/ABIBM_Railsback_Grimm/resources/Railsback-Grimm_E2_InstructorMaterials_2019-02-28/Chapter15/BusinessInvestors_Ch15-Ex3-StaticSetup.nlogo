globals
 [
   ...
   ; Investor parameters
   ; decision-time-horizon  Moved to slider
  ]
...
to setup
 ...
 ; Initialize global variables
  ...
  ; set decision-time-horizon 5  Moved to slider

 ; Initialize patch profit and risk characteristics
 with-local-randomness  ; Now, make sure we get same 'random' setup each time
 [
  random-seed 6874
  ask patches
    [
...
@#$#@#$#@
# BusinessInvestors_Ch15-Ex3-StaticSetup.nlogo
...
This is the first version of the Business Investor model, as described in Section 10.4.1. It is modified for Exercise 3 of Chapter 15 so that patches and turtles are initialized with the same values each model run, by using `with-local-randomness` and a random seed in the `setup` procedure.
...
@#$#@#$#@
<experiments>
  <experiment name="Time-horizon" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>

...
to setup
  ...
  ; NEW: Create "streets"
  ask patches with [(remainder pxcor street-spacing) = 0]
  [ set pcolor grey]
  ask patches with [(remainder pycor street-spacing) = 0]
  [ set pcolor grey]


  ;; create turtles on random patches.
  ;; Now, include only patches not on streets
...
;; New *patch* reporter to say whether a turtle would be happy there
to-report is-happy? [a-turtle]  ; a boolean patch reporter

  let num-similar-nearby count (turtles-on neighbors) with [color = [color] of a-turtle]
  let num-nearby count turtles-on neighbors
  ifelse num-nearby > 0
  [ report (num-similar-nearby / num-nearby) >= (%-similar-wanted / 100) ]
  [ report true ] ; assume turtle is happy if no neighbors

end

;; move until we find an unoccupied spot
;; Now streets are also excluded.
to find-new-spot
  rt random-float 360
  fd random-float 10
  if (any? other turtles-here) or (pcolor = grey) [ find-new-spot ] ;; keep going until we find an unoccupied patch

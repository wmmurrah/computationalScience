globals
 ...
 ;  money-size-ratio   --Moved to slider
  ...
  call-list    ; Ch.14 Ex 8  a list of all the calls made each time step, by telemarketer
 ]

turtles-own
 [
   weekly-sales   ; Ch.14 Ex 8
   weekly-cost
   bank-balance
   potential-customers ; Ch.14 Ex 8 patches within the call radius
 ]
...
to go
  ...
  set-up-calls ; Ch.14 Ex 8

;  file-open "TelemarketerSalesTest.csv"
  ; Ch.14 Ex 8  The following statement replaces "ask turtles"
  ; call-list is a list of telemarketers with one entry per call to be made
  show "Making call # 1"
  foreach call-list
  [
    next-caller -> ask next-caller [ make-a-call ]
  ]
  show (word "Finished call # " (length call-list))
...
to set-up-calls ; Ch.14 Ex 8  a global procedure

  set call-list []

  ask turtles
  [
    let num-calls 100 * size
    set potential-customers patches in-radius (10 * size ^ 0.5)
    if count potential-customers < num-calls [ set num-calls count potential-customers ]
    repeat num-calls [ set call-list fput self call-list ]
    ; show length call-list

    set weekly-sales 0
  ]

  set call-list shuffle call-list

end


to make-a-call   ; Ch.14 Ex 8  A turtle procedure

  ; show " calling"

  ; Identify a potential buyer
  let the-buyer one-of potential-customers

  ; See whether a sale is made
  if [pcolor] of the-buyer = green ; This buyer has not yet been called so we make a sale
  [
    set weekly-sales weekly-sales + 1
    set total-sales total-sales + 1

    ask the-buyer [set pcolor brown]
  ]

  ; Remove potential buyer from potential customers because we do not call the same
  ; household more than once per time step
  set potential-customers potential-customers with [self != the-buyer]
...
@#$#@#$#@
# Telemarketer_Ch14-Ex8_2ndEd.nlogo
...
This version is the first, basic version of the model modified as a solution to Exercise 8 of Chapter 14. This version has telemarketer calls scheduled so that all the telemarketers take turns calling customers throughout the time step, instead of having each telemarketer make all its calls before the next one executes its calls. Changes are marked in the code with this comment: `; Ch.14 Ex 8`.
...
The order in which the telemarketers do this second action is obviously important because the first marketer to call a customer makes a sale, whereas marketers who call later in the same week make no sale. **In this version, the telemarketers make individual calls to potential customers, with the order in which the telemarketers make calls randomized.**

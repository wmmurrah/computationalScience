globals
 [
   ...
   ; Variables for Bayesian update of estimated risk
   initial-risk-alpha
   initial-risk-beta
...
patches-own
 [
  ...
  p-num-years-occupied  ; Variables for Bayesian update of estimated risk
  p-num-failures
 ]
...
to setup
 ...
 ; Initialize global variables
  ...
  ; Initialize Bayesian updating variables
  let R (patch-max-risk + patch-min-risk) / 2        ; R is initial mean risk
  let V ((patch-max-risk - patch-min-risk) ^ 2) / 12 ; V is initial variance in risk
  set initial-risk-alpha ((R ^ 2) - (R ^ 3) - (R * V)) / V
  set initial-risk-beta ((R / V) * ((1 - R) ^ 2)) + (R - 1)

  show (word "Mean risk: " R "  Variance in risk: " V)
  ...
  ask patches
    [
     ...
     set plabel precision annual-risk 2
      ...
      ; Initialize Bayesian update variables
 ...
 ; This file is now used to test the Bayesian updating
...
;      file-type "tick,"
;      file-type "initial-risk-alpha,"
;      file-type "initial-risk-beta,"
;      file-type "years-occupied,"
;      file-type "num-failures,"
;      file-type "est-risk,"
;      file-print "real-risk"
;   file-close
...
to do-accounting
  ...
  ; Count failures and update patch variable for num failures
  if wealth = 0.0
  [
    set num-investor-failures num-investor-failures + 1
    set p-num-failures p-num-failures + 1
  ]

  ; Update patch variable for num years occupied
  set p-num-years-occupied p-num-years-occupied + 1
...
to-report utility-for [a-turtle]
 ...
 ; In this version, estimate risk using Bayesian updating formula

 let alpha initial-risk-alpha + p-num-failures
 let beta initial-risk-beta + p-num-years-occupied - p-num-failures

 let estimated-risk alpha / (alpha + beta)

 set utility utility * ((1 - estimated-risk) ^ decision-time-horizon)

 ; Now write test output -- comment these output statements out after testing

;  file-open "TestOutput-Investors.csv"
;    file-type [who] of a-turtle          file-type ","
;    file-type ticks          file-type ","
;    file-type initial-risk-alpha          file-type ","
;    file-type initial-risk-beta          file-type ","
;    file-type p-num-years-occupied          file-type ","
;    file-type p-num-failures          file-type ","
;    file-type estimated-risk          file-type ","
;    file-print annual-risk
;  file-close
...
@#$#@#$#@
# BusinessInvestors_Ch12-Ex3_2ndEd.nlogo
...
This is the first version of the model, as described in Section 10.4.1. However, the code implements the Bayesian updating method for Exercise 3, Chapter 12. Risk is estimated from the number of failures previously experienced. This change is not included in the description below.
...
@#$#@#$#@
<experiments>
  <experiment name="Reps-BayesUpdating" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>

...
;  set punishment-probability 0.2   ; moved to interface; the probability of a harvester being punished when taking more than cooperative harvesters
  ...
  [
    ...
    file-print "Tick,Num-cooperative,Num-selfish,Total-resource,Total-resource-growth,Cumulative-resource-growth,Cumulative-coop-harvest,Cumulative-selfish-harvest"
    file-close
  ]
...
;  if ticks = 1 [file-print "Who,energy,behavior-type,max-resource,resource,greediness,num-harvesters,cooperative-harvest,selfish-harvest,harvest"]  ; Header for test output
  ask turtles [ harvest-resource ]
;  file-close  ; Temporary test output

  ask selfish-harvesters [ get-punished ]

  ask turtles [ resettle ]
...
;  let prev-resource resource - harvest-here-last-tick
  ...
  set resource-growth (resource-growth-rate * resource * (1 - (resource / max-resource)))
  ...
  set resource resource + resource-growth

  ; Some defensive programming: resource should not be negative
  if resource < 0.0
  [
    inspect self
    error (word "Resource is negative with prev-resource: " resource " resource-growth: " resource-growth)
  ]
...
to harvest-resource ; A turtle procedure
  ...
  if harvest > resource [ set harvest resource ]

  set energy energy + harvest

  ; Temporary test output
;  file-print (word who "," old-energy "," behavior-type "," max-resource "," resource "," greediness ","
;    (count turtles-here) "," cooperative-harvest "," selfish-harvest "," harvest)

  set resource resource - harvest
...
to resettle  ; A procedure executed by all harvesters

  ; Set a boolean variable for whether harvester is "dissatisfied"
  let im-not-satisfied? ifelse-value (behavior-type = "cooperative")
  [ harvest < maintenance-energy ] ; Report true or false for cooperative harvesters
  [ harvest < selfish-harvest ]    ; Report true or false for selfish harvesters

  if im-not-satisfied? and (random-float 1.0 > dissatisfaction-resettle-prob)
  [
...
to reproduce ; A turtle procedure
  ...
  [
    ...
    [
      ...
      set energy [energy] of myself ; Offspring's energy is set to parent's, which was halved above.
      set harvest 0.0               ; Initialize all the harvester's variables, to be safe
      set cooperative-harvest 0.0
      set selfish-harvest 0.0
    ] ; hatch
  ] ; if reproduce
...
to update-outputs
  ...
  ; We cannot determine the total harvest by summing over harvesters here because some died and some were born
  if file-output?
  [
    file-open "HarvesterOutput.csv"
...
@#$#@#$#@
# HARVESTER MODEL
This file is copyrighted 2019 by Steven F. Railsback and Volker Grimm, and provided as instructor materials for Chapter 6 of _Agent-based and Individual-based Modeling: A Practical Introduction, 2nd Edition_. Please do not copy or distribute this file; it is available upon request from www.railsback-grimm-abm-book.com.
...
This NetLogo file has the known programming errors corrected, and this ODD description includes changes identified during the software testing exercise.
...
Initialization begins by setting the value of _max-resource_ of each patch. The user selects one of four optional distributions of this variable, with the distribution treated as a model parameter. Upon initialization, the value of _max-resource_ for each patch is drawn randomly from the selected distribution. For all four distributions, the mean of _max-resource_ is a model parameter _max-resource-mean_, which has a standard value of 100 energy units. The four alternative distributions are:
  ...
  * Normal: _max-resource_ is drawn from a normal distribution with mean of _max-resource-mean_ and standard deviation of 1/3 _max-resource-mean_. However, if the resulting value of _max-resource_ is negative, it is set to 1.0.
...
Two changes to this model description are added as a result of code testing. First, for all harvesters, the harvest is also limited by the available resource: If _harvest_ is greater than the value of _resource_ for the harvester's patch, then _harvest_ is set equal to _resource_. Second, harvesters deplete their patch's resource as they harvest it: at the end of this submodel, the patch's value of _resource_ is reduced by the value of _harvest_.

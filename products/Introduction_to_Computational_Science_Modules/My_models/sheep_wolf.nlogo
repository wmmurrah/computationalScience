...
turtles-own [
  energy
]  ;; All agents own energy

patches-own [ ;; patches have an amount of grass
  grass-amount
]
...
to setup
  ...
  ask patches [ ;; color the world green
    ...
    ;; Color it shades of green based on amount of grass.
    recolor-grass
  ]
  create-sheep number-of-sheep [ ;; create initial sheep
    setxy random-xcor random-ycor
    set color white
    set shape "sheep"
    set energy 100
  ]
  create-wolves number-of-wolves [  ;; create initial wolves
    setxy random-xcor random-ycor
    set color brown
    set shape "wolf"
    set size 1.5      ;; Increase their size so they are easier to see.
    set energy 100    ;; set initial energy
  ]
  reset-ticks
end
...
to go
  ...
  ask turtles [
    wiggle
    move
    check-if-dead ;; check if sheep dies
    eat
    reproduce
  ]
  regrow-grass
...
;; The sheep turn in random directions.
to wiggle
...
;; Sheep procedure: the sheep moves which costs energy.
to move
  forward 1
  set energy energy - movement-cost ;; take away one unit of energy per move.
end

;; If a sheep's energy is low, it dies.
to check-if-dead
  if energy < 0 [
    die
  ]
...
;; Sheep procedure: eat grass
to eat-grass
  ;; Check to make sure there is grass here
  if (grass-amount >= 1) [
    ;; increment sheep energy
...
to eat-sheep
  if any? sheep-here [  ;; if there are sheep here then eat one.
    let target one-of sheep-here  ;; select random sheep on occuppied patch
    ask target [  ;; eat selected sheep
      die
    ]
...
;; Recolor grass to indicate how much has been eaten.
to recolor-grass
  set pcolor scale-color green grass-amount 0 20
end

;; Grass procedure: regrow grass
to regrow-grass
  ask patches [
...
;; Check to see if this sheep has enough enery to reproduce
to reproduce
  if energy > 200 [
...
"wolves" 1.0 0 -10402772 true "" ""
"grass" 1.0 0 -15040220 true "" ""
